...
const SalesOrderRegister: React.FC = () => {
  ...
  const [selectedVoucher, setSelectedVoucher] = useState<SalesInvoice | null>(
    null,
  );
  ...
  const [selectedInvoiceForEwb, setSelectedInvoiceForEwb] =
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (ledgersRes.success && ledgersRes.data && ledgersRes.data.ledgers) {
          const bank = ledgersRes.data.ledgers.find(
            (l: any) =>
  ...
  const allParties = useMemo(
    () => [
      "All",
      ...new Set(invoices.map((d) => d.party_name).filter(Boolean)),
    ],
    [invoices],
  );
  ...
  const filteredPartyList = useMemo(() => {
    ...
    return allParties.filter((p) =>
      p.toLowerCase().includes(partySearchQuery.toLowerCase()),
    );
  ...
  const filteredSalesData = useMemo(() => {
    ...
    return invoices.filter((item) => {
      ...
      const matchesStartDate =
        !startDate || itemDate >= new Date(startDate).getTime();
      const matchesEndDate =
        !endDate || itemDate <= new Date(endDate).getTime();
      const matchesStatus =
  ...
  const toggleSelectBill = (voucherNo: string) => {
    setSelectedBills((prev) =>
      prev.includes(voucherNo)
        ? prev.filter((b) => b !== voucherNo)
        : [...prev, voucherNo],
    );
  };
  ...
  const toggleSelectAll = () => {
    if (
      selectedBills.length === filteredSalesData.length &&
  ...
  const handleCancelEwb = async () => {
    if (!selectedInvoiceForEwb || !selectedInvoiceForEwb.ewaybill?.ewb_no)
      return;
      ...
      if (!tokenRes.success || !authToken) {
        alert(
          "Failed to get E-Way Bill Access Token: " +
            (tokenRes.message ||
              "Token missing in response. Please check settings."),
        );
        setCancelLoading(false);
        return;
      }
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        const transformedVoucher = {
          ...
          einvoice: {
            status: res.data.einvoice_status as
              | "not_generated"
              | "generated"
              | "failed",
          ...
          ewaybill: {
            status: res.data.ewaybill_status as
              | "not_generated"
              | "generated"
              | "failed",
  ...
  const handleBulkExport = async () => {
    ...
    try {
      const selectedInvoices = invoices.filter((inv) =>
        selectedBills.includes(inv.voucher_no),
      );
      ...
      detailedInvoices.forEach((inv) => {
        const partyStateCode =
          inv.party_gstin?.substring(0, 2) || companyStateCode;
      ...
      link.setAttribute(
        "download",
        `sales_export_gst_${new Date().toISOString().split("T")[0]}.csv`,
      );
  ...
  const handleGenerateEInvoice = async (voucherId: number) => {
    try {
      const response = await fetch(
        `/gst/einvoice_generate.php?voucher_id=${voucherId}`,
      );
      if (response.ok) {
  ...
  const handleGenerateEWayBill = async (voucherId: number) => {
    try {
      const response = await fetch(
        `/gst/ewaybill_generate.php?voucher_id=${voucherId}`,
      );
      if (response.ok) {
        const data = await response.json();
        if (data.success) {
  ...
  const taxableValue = useMemo(() => {
    ...
    return selectedVoucher.items.reduce(
      (acc, item) => acc + parseFloat(item.quantity) * parseFloat(item.rate),
      0,
    );
  }, [selectedVoucher]);
    ...
    <div className="space-y-8 animate-in fade-in duration-700 max-w-[1600px] mx-auto pb-24 px-4 sm:px-0">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => navigate("/vouchers/sales-order")}
      ...
      <div className="hidden lg:grid grid-cols-4 gap-6">
        <div className="md:col-span-2 glass-panel p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/40 flex items-center gap-6 border-white/50">
          ...
          <button
            ...
            className={`p-4 border rounded-[1.5rem] transition-all transform active:scale-90 ${showFilters ? "bg-indigo-600 text-white border-indigo-600 shadow-lg shadow-indigo-100" : "bg-white border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-indigo-600 shadow-sm"}`}
          >
            <Filter size={24} />
          </button>
        </div>
      ...
      <div className="grid lg:hidden grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="sm:col-span-2 glass-panel p-5 rounded-[2rem] shadow-xl shadow-slate-200/40 flex items-center gap-3 border-white/50">
          ...
          <button
            onClick={() => setShowFilters(!showFilters)}
            className={`p-3 border rounded-2xl transition-all transform active:scale-90 ${showFilters ? "bg-indigo-600 text-white border-indigo-600 shadow-md shadow-indigo-100" : "bg-white border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-indigo-600 shadow-sm"}`}
          >
            <Filter size={20} />
          </button>
        </div>
        ...
        <div className="bg-emerald-50/50 p-5 rounded-[2rem] border border-emerald-100/50 flex items-center justify-between group hover:shadow-md transition-all">
          <div>
            ...
            <h3 className="text-xl font-black text-slate-900 mt-0.5 tracking-tighter">
              ₹{totalFilteredAmount.toLocaleString()}
            </h3>
          </div>
        ...
        <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-2xl shadow-indigo-500/5 animate-in slide-in-from-top-6 duration-500">
          <div className="flex items-center justify-between mb-10 pb-6 border-b border-slate-50">
            ...
            <button
              ...
              className="text-[10px] font-black text-rose-500 uppercase tracking-widest hover:text-rose-600 px-4 py-2 bg-rose-50 rounded-lg hover:bg-rose-100 transition-all"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            <div className="space-y-3 relative" ref={partyDropdownRef}>
              ...
              <div
                ...
                className="w-full bg-slate-50/50 border border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold flex justify-between items-center cursor-pointer hover:border-indigo-300 transition-all"
              >
                <span
                  className={
                    filterParty === "All" ? "text-slate-400" : "text-slate-900"
                  }
                >
                  {filterParty}
                </span>
                  ...
                  <div className="max-h-56 overflow-y-auto space-y-1 custom-scrollbar pr-2">
                    {filteredPartyList.map((p) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 rounded-xl text-xs font-bold transition-all ${filterParty === p ? "bg-indigo-600 text-white shadow-lg shadow-indigo-100" : "text-slate-600 hover:bg-indigo-50 hover:text-indigo-600"}`}
                      >
                        {p}
                      </button>
                    ))}
          ...
          <div className="mt-8 pt-8 border-t border-slate-100">
            <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1 mb-4 block">
              Bill Status
            </label>
      ...
      <div className="bg-white rounded-[3rem] border border-slate-200 shadow-xl shadow-slate-200/30 overflow-hidden bento-item">
              ...
              <tr>
                <th className="px-8 py-8 w-[80px] text-center">
                  <button
                    ...
                    className={`w-6 h-6 rounded-lg border-2 transition-all flex items-center justify-center ${selectedBills.length === filteredSalesData.length && filteredSalesData.length > 0 ? "bg-indigo-600 border-indigo-600" : "border-slate-200 bg-white hover:border-indigo-300"}`}
                  >
                    {selectedBills.length > 0 && (
                      <Check
                        size={14}
                        className="text-white mx-auto stroke-[4]"
                      />
                    )}
                  </button>
                </th>
                ...
                <tr>
                  <td colSpan={9} className="py-40 text-center">
                    <div className="flex flex-col items-center gap-4 text-slate-300">
                      ...
                      <button
                        onClick={resetFilters}
                        className="mt-4 px-6 py-3 bg-slate-100 text-slate-500 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all shadow-sm"
                      >
                        Clear All Filters
                      </button>
                    </div>
                ...
                filteredSalesData.map((bill) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <button
                          onClick={() => toggleSelectBill(bill.voucher_no)}
                          className={`w-6 h-6 rounded-lg border-2 transition-all flex items-center justify-center ${isSelected ? "bg-indigo-600 border-indigo-600" : "border-slate-100 bg-white group-hover:border-indigo-200"}`}
                        >
                          {isSelected && (
                            <Check
                              size={14}
                      ...
                      <td className="px-4 py-6">
                        <div className="flex flex-col">
                          <span className="text-[11px] font-black text-slate-900 tracking-tight">
                            {new Date(bill.voucher_date).toLocaleDateString(
                              "en-GB",
                              {
                                day: "2-digit",
                                month: "short",
                          ...
                          <span className="text-[9px] font-bold text-slate-400 uppercase mt-0.5">
                            {new Date(bill.voucher_date).toLocaleTimeString(
                              [],
                      ...
                      <td className="px-4 py-6">
                        <span className="px-3 py-1.5 bg-indigo-50 text-indigo-600 text-[11px] font-black tracking-widest rounded-lg border border-indigo-100 shadow-sm">
                          {bill.voucher_no}
                        </span>
                      </td>
                      <td className="px-4 py-6">
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 font-black group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-inner uppercase">
                            {(bill.party_name || "U")[0]}
                          </div>
                          <div className="flex flex-col min-w-0">
                            ...
                            <span className="text-[10px] font-black text-slate-400 uppercase mt-0.5 tracking-tighter truncate">
                              GST: {bill.party_gstin || "UNREGISTERED"}
                            </span>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-6 text-center">
                        <div className="flex flex-col items-center gap-2">
                          {bill.einvoice?.status === "generated" &&
                          bill.einvoice?.irn ? (
                            <div className="flex flex-col items-center">
                              ...
                              />
                              <span className="text-[8px] font-black uppercase mt-1 text-emerald-600">
                                Generated
                              </span>
                              <span
                                className="text-[7px] font-bold text-emerald-600 mt-0.5 bg-emerald-50 px-2 py-0.5 rounded max-w-[90px] truncate"
                                title={bill.einvoice.irn}
                              >
                                {bill.einvoice.irn}
                              </span>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              ...
                              <span className="text-[8px] font-black uppercase mt-1 text-red-600">
                                Failed
                              </span>
                            </div>
                          ) : (
                            ...
                            >
                              ...
                              />
                              <span className="text-[7px] font-black uppercase text-slate-400 group-hover:text-amber-600">
                                Generate
                              </span>
                            </button>
                          )}
                        ...
                        <div className="flex flex-col items-center gap-2">
                          ...
                          bill.ewaybill?.ewb_no ? (
                            ...
                            >
                              <Navigation
                                size={16}
                                className="text-indigo-500 fill-indigo-50 rotate-45 group-hover:scale-110 transition-transform"
                              />
                              <span className="text-[8px] font-black uppercase mt-1 text-indigo-600">
                                Generated
                              </span>
                              <div className="flex items-center gap-1 mt-0.5">
                                <span
                                  className="text-[7px] font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded max-w-[70px] truncate"
                                  title={bill.ewaybill.ewb_no}
                                >
                                  {bill.ewaybill.ewb_no}
                                </span>
                                <button
                            ...
                            >
                              <Navigation
                                size={16}
                                className="text-slate-300 group-hover:text-indigo-500 rotate-45"
                              />
                              <span className="text-[7px] font-black uppercase text-slate-400 group-hover:text-indigo-600">
                                Generate
                              </span>
                            </button>
                          )}
                ...
                <div className="flex gap-2 pt-2 justify-end">
                  ...
                  {bill.ewaybill?.status === "generated" ? (
                    ...
                    >
                      <Navigation
                        size={16}
                        className="text-indigo-500 fill-indigo-100 rotate-45 group-hover:scale-110 transition-transform flex-shrink-0"
                      />
                      <div
                        onClick={(e) => {
                          e.stopPropagation();
        ...
        <div className="p-10 bg-slate-900 text-white flex flex-col md:flex-row items-center justify-between gap-8 border-t border-white/5 hidden lg:flex">
          <div className="flex items-center gap-8">
            <div className="flex flex-col">
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400 mb-1">
                Total Turnover
              </span>
            ...
            <div className="flex flex-col">
              ...
              <span className="text-xl font-black">
                {filteredSalesData.length}{" "}
                <span className="text-xs opacity-50">Vouchers</span>
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="p-10 bg-slate-900 text-white flex flex-col md:flex-row items-center justify-between gap-8 border-t border-white/5 md:flex lg:hidden">
          <div className="flex items-center gap-8">
            <div className="flex flex-col">
              <span className="text-[8px] font-black uppercase tracking-[0.2em] text-indigo-400 mb-1">
                Total Turnover
              </span>
              <span className="text-3xl font-black tracking-tighter">
                ₹{totalFilteredAmount.toLocaleString()}
          ...
          <div className="flex items-center gap-4">
            <button className="px-6 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all">
              Export CSV
            </button>
          </div>
        </div>
        ...
        {selectedBills.length > 0 && (
          <div className="fixed bottom-10 left-1/2 -translate-x-1/2 w-[90%] max-w-[1000px] bg-slate-900 rounded-[3rem] p-6 shadow-2xl z-50 animate-in slide-in-from-bottom-12 duration-500 flex items-center justify-between border border-white/10 backdrop-blur-2xl bg-opacity-95">
            <div className="flex items-center gap-8 pl-4">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-[1.5rem] bg-indigo-600 flex items-center justify-center font-black text-white text-xl shadow-2xl shadow-indigo-500/40">
                ...
                <div className="flex flex-col">
                  ...
                  <button
                    ...
                    className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mt-1 hover:text-indigo-300 transition-colors"
                  >
                    Deselect All
                  </button>
                </div>
            ...
            <div className="relative w-full h-full sm:h-auto max-w-5xl bg-white sm:rounded-[3rem] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col max-h-[100dvh] sm:max-h-[95vh] animate-in zoom-in-95 duration-300">
              <div className="p-4 sm:p-6 border-b border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white sticky top-0 z-20">
                <div className="flex items-center justify-between w-full sm:w-auto">
                  ...
                  <button
                    onClick={() => setIsViewModalOpen(false)}
                    className="p-2 sm:hidden text-slate-400 hover:text-rose-500 transition-all bg-slate-50 rounded-xl"
                  >
                    <X size={20} />
                  </button>
                </div>
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0">
                  <button
                    onClick={handlePrint}
                    className="flex-1 sm:flex-none justify-center px-4 sm:px-6 py-2.5 sm:py-3 bg-slate-100 text-slate-600 rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all flex items-center gap-2 whitespace-nowrap"
                  >
                    <Printer size={14} className="sm:w-4 sm:h-4" /> Print
                  </button>
                  <button
                    onClick={handleDownloadPDF}
                    className="flex-1 sm:flex-none justify-center px-4 sm:px-6 py-2.5 sm:py-3 bg-indigo-600 text-white rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 flex items-center gap-2 whitespace-nowrap"
                  >
                    <FileDown size={14} className="sm:w-4 sm:h-4" /> Save PDF
                  </button>
                  <button
                    onClick={() => setIsViewModalOpen(false)}
                    className="hidden sm:block p-3 text-slate-400 hover:text-rose-500 transition-all ml-2 bg-slate-50 rounded-2xl flex-shrink-0"
                  >
                    <X size={20} />
                  </button>
                </div>
              ...
              <div className="p-4 sm:p-8 border-t border-slate-100 bg-white flex flex-col sm:flex-row items-center gap-3 sm:gap-4 z-20 pb-safe">
                <button
                  onClick={() => setIsViewModalOpen(false)}
                  className="w-full sm:w-auto px-6 sm:px-16 py-3 sm:py-4 bg-slate-900 text-white rounded-xl sm:rounded-[1.5rem] text-[10px] sm:text-[10px] font-black uppercase tracking-widest shadow-xl sm:shadow-2xl shadow-slate-900/30 hover:bg-black transition-all active:scale-95 flex items-center justify-center gap-2"
                >
                  Close
                </button>
                ...
                <button
                  onClick={() => {
                    ...
                    if (selectedVoucher) handleEdit(selectedVoucher);
                  }}
                  className="w-full sm:w-auto px-6 sm:px-12 py-3 sm:py-4 bg-slate-100 text-slate-700 rounded-xl sm:rounded-[1.5rem] text-[10px] sm:text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all active:scale-95 shadow-sm"
                >
                  Edit
                </button>
              ...
              <div key={inv.id} className="invoice-page">
                <InvoiceTemplate
                  ...
                  taxableValue={
                    inv.items?.reduce(
                      (acc: number, item: any) =>
                        acc + parseFloat(item.quantity) * parseFloat(item.rate),
                      0,
                    ) || 0
                  }
                  totalTaxAmount={
                    inv.items?.reduce(
                      (acc: number, item: any) =>
                        acc + parseFloat(item.tax_amount || "0"),
                      0,
                    ) || 0
                  }
                  businessDetails={businessDetails}

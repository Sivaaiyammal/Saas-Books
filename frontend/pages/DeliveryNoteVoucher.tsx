          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-lg font-bold text-slate-900 leading-tight">
                {isEditing ? "Edit Sales Voucher" : "Sales Voucher"}
              </h1>
              <span
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-lg font-bold text-slate-900 leading-tight">
                  {isEditing ? "Edit Sales" : "Sales Voucher"}
                </h1>
                <span
      ...
      <div className="flex flex-col xl:grid xl:grid-cols-12 gap-6">
        ...
        <div className="xl:col-span-12 bg-white p-4 sm:p-6 md:p-8 rounded-2xl md:rounded-3xl border border-slate-200 shadow-sm space-y-6 md:space-y-8">
          ...
          <div className="flex flex-wrap items-center gap-4 sm:gap-8 md:gap-12 pt-4 border-t border-slate-50">
            ...
            {/* <div
              className="flex items-center gap-2 cursor-pointer group"
              onClick={() => setIsEInvoice(!isEInvoice)}
            >
              <div
                className={`w-10 h-6 rounded-full transition-all relative shrink-0 ${isEInvoice ? "bg-emerald-600" : "bg-slate-200"}`}
              >
                <div
                  className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all shadow-sm ${isEInvoice ? "right-1" : "left-1"}`}
                />
              </div>
              <span className="text-[9px] sm:text-[10px] font-black uppercase text-slate-500 tracking-wider group-hover:text-emerald-600 transition-colors">
                E-Invoice
              </span>
            </div>
            <div
              className="flex items-center gap-2 cursor-pointer group"
              onClick={() => setIsEWayBill(!isEWayBill)}
            >
              <div
                className={`w-10 h-6 rounded-full transition-all relative shrink-0 ${isEWayBill ? "bg-amber-600" : "bg-slate-200"}`}
              >
                <div
                  className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all shadow-sm ${isEWayBill ? "right-1" : "left-1"}`}
                />
              </div>
              <span className="text-[9px] sm:text-[10px] font-black uppercase text-slate-500 tracking-wider group-hover:text-amber-600 transition-colors">
                E-Way Bill
              </span>
            </div> */}
          </div>
